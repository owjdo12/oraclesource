...
--실습 SQL
--EMP테이블에서 사원들은 입사일기준 3개월이 지난 후 첫 월요일에 정직원이 된다. 사원들이 정직원이 되는날짜(R_JOB)를
--YYYY-MM-DD형식을 아래와 같이 출력하시오. 단, 추가수당(COMM)이 없는사원의 추가수당은 N/A로 출력하시오
SELECT * FROM emp;

-- EMP테이블의 모든 사원을 대상으로 직속 상관의 사원번호(MGR)를 다음과 같은 조건을 기준으로 변환해서 CHG_MGR열에 출력하시오
--직속상관의 사원번호가 존재X:0000, 앞두자리 75: 5555, 76: 6666, 77: 7777, 78: 8888, 그 외: 상관의 사원번호 그대로
SELECT empno, ename, mgr, CASE WHEN EMPNO IS NULL THEN 0000 WHEN EMPNO LIKE '75%' THEN 5555 WHEN EMPNO LIKE '76%' THEN 6666 WHEN EMPNO LIKE '77%' THEN 7777 WHEN EMPNO LIKE '78%' THEN 8888 ELSE EMPNO END AS chg_mgr FROM emp;
